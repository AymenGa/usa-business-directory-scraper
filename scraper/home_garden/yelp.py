"""
Home and Garden — Yelp business scraper (directory data).
Scrapes Yelp search results for home/garden businesses.
3 businesses per city × 250 cities = 750 rows.
Visits individual business pages when phone/website missing from search.

Run:
  python scraper/home_garden/yelp.py --test        # first 5 cities
  python scraper/home_garden/yelp.py --state TX    # one state only
  python scraper/home_garden/yelp.py               # all 250 cities
"""
...
PROGRESS_FILE  = os.path.join(CATEGORIES_DIR, "home_garden_yelp_progress.json")

CATEGORY_SLUG  = "home_and_garden"
AUDIT_FILE     = os.path.join(CATEGORIES_DIR, "home_and_garden_yelp_categories_audit.json")
REJECTED_FILE  = os.path.join(CATEGORIES_DIR, "yelp_rejected_all.json")

# Grey-zone keywords — accepted rows containing these are flagged suspicious in audit
SUSPICIOUS_KEYWORDS = {
    "gift", "craft", "fabric", "art", "discount", "bargain",
    "consign", "thrift", "florist", "flower", "book", "hobby",
    "plant", "nursery",
}
...
# ── Category filters ─────────────────────────────────────────────────────────
# Categories that belong to Home and Garden (STORES that SELL home/garden products)
ALLOWED_CATEGORIES = {
    "home decor", "furniture", "furniture stores",
    "garden centers", "nurseries & gardening",
    "kitchen & bath", "kitchen supplies", "kitchenware",
    "lighting stores", "lighting fixtures & equipment",
    "home & garden", "home organization",
    "rugs", "carpeting", "flooring",
    "mattresses", "bedding",
    "outdoor furniture stores",
    "antiques", "vintage",
    "appliances",
    "hardware stores", "nurseries",
    "candle stores",
    "tableware", "cookware",
    "fireplaces", "fireplace services",
    "blinds", "shades", "curtains",
    "storage", "closet designers",
    "hot tub & pool", "patio coverings",
    "home accessories", "wall art",
}

# Hard reject — NEVER home & garden, even if a matching tag is present
REJECT_CATEGORIES = {
    # Service contractors (not stores)
    "landscaping", "landscape architects",
    "tree services", "lawn services", "lawn care",
    "pest control", "exterminators",
    "plumbing", "plumbers",
    "electricians", "electrical",
    "heating & air conditioning/hvac", "hvac",
    "roofing", "contractors", "general contractors",
    "handyman", "home inspectors",
    "movers", "moving", "junk removal",
    "carpet cleaning", "house cleaning", "cleaning",
    "locksmiths", "security systems",
    "painting", "painters",
    ...
    # Craft / hobby / gift — not home & garden
    ...
    "discount store", "discount stores",
    "bookstores", "books", "book stores",
    "toy stores", "toys",
    ...
    # Art / gallery — not home & garden stores
    "art galleries", "art gallery", "framing",
    "art classes", "art schools",
    # Non-profit / community service
    "community service/non-profit", "non-profit", "nonprofit",
}
...
SERVICE_CATEGORIES = {
    ...
    "furniture reupholstery", "furniture repair",
    "appliance repair",
    "window washing", "pressure washing",
    "garage door services",
}

# Business names that signal non-home-garden even if categories look OK
REJECT_NAME_KEYWORDS = {
    # Service contractors
    "landscaping", "landscape", "lawn care", "lawn service",
    "tree service", "tree care", "tree trimming",
    "plumber", "plumbing", "electrician", "electrical",
    "hvac", "heating", "cooling", "air conditioning",
    "roofing", "roofer", "contractor", "construction",
    "handyman", "handy man",
    "pest control", "exterminator",
    "cleaning", "cleaners", "maid",
    "painting", "painter", "painters",
    "moving", "movers", "hauling", "junk removal",
    "locksmith", "security",
    # Non-store businesses
    ...
    # Wrong-category chain stores that keep slipping through
    "joann", "jo-ann", "hobby lobby",
    "florist", "flower shop", "cut flower",
    # Art galleries
    "gallery", "galleries",
    # Non-profits
    "habitat for humanity", "non-profit", "nonprofit",
    # Clothing
...
# ── Category matching ────────────────────────────────────────────────────────
def is_home_garden_business(categories_text):
    """Check if the business categories match Home and Garden STORES.

    Returns (accepted: bool, reject_reason: str).
    reject_reason is the category keyword that triggered rejection, or "" if accepted.

    Logic:
    1. Hard reject if any category is in REJECT_CATEGORIES (landscaping, auto, etc.)
    2. Check if business has any ALLOWED (selling) category
    3. If it ONLY has service categories and NO selling categories → reject
    4. If it has a mix of service + selling categories → accept
    """
    if not categories_text:
        return False, "empty"
    cats = [c.strip().lower() for c in categories_text.split(",")]

    # Reject if primary category (first) is accessories — fashion store, not home
    if cats and cats[0] in {"accessories", "women's clothing", "men's clothing", "clothing"}:
        return False, cats[0]

    # Step 1: Hard reject — never home & garden regardless
    for c in cats:
    ...
    for link in biz_links:
        ...
        for span in cat_spans:
            ...
            if "," in text and any(kw in text.lower() for kw in
                ["furniture", "home decor", "garden", "kitchen",
                 "lighting", "nursery", "rug", "antique",
                 "appliance", "bath", "flooring", "hardware"]):
                categories = text
                break
...
def extract_business_details(html):
    ...
    for link in soup.find_all("a", href=re.compile(r"biz_redir")):
        href = link.get("href", "")
        # Prefer the real URL embedded in the redirect parameter
        url_match = re.search(r"url=([^&]+)", href)
        if url_match:
            from urllib.parse import unquote
            details["website"] = unquote(url_match.group(1))
            break
        # Fallback: link text only if it looks like a URL
...
# ── Rejected businesses JSON helpers ─────────────────────────────────────────
def load_rejected():
    """Load existing rejected records from the shared file."""
...
def save_rejected(records):
    os.makedirs(CATEGORIES_DIR, exist_ok=True)
    with open(REJECTED_FILE, "w", encoding="utf-8") as f:
        json.dump(records, f, indent=2, ensure_ascii=False)


def make_audit_record(state, city, biz):
    """Build an audit record for one accepted business."""
    cats = biz.get("categories", "").lower()
    suspicious = any(kw in cats for kw in SUSPICIOUS_KEYWORDS)
    return {
        "state": state,
        "city": city,
        "name": biz.get("name", ""),
...
def merge_directory():
    ...
    # Load existing directory rows (Etsy + AptDeco + any other non-Yelp sources)
    if os.path.exists(etsy_path):
        ...
        for r in range(2, ws.max_row + 1):
            ...
            if source and source != "Yelp":  # include all non-Yelp rows
    ...
    log.info(f"Merged directory: {summary} = {len(all_rows)} total → {final_path}")


# ── Main ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Home and Garden — Yelp scraper")
    ...
    rejected_records = load_rejected()
    ...
    try:
        for idx, (state, city) in enumerate(remaining, 1):
            ...
            try:
                ...
                for query_text in SEARCH_QUERIES:
                    ...
                    for page in range(MAX_PAGES):
                        ...
                        if idx == 1 and page == 0 and query_text == SEARCH_QUERIES[0]:
                            debug_path = os.path.join(CATEGORIES_DIR, "_debug_home_garden_yelp_search.html")
                        ...
                        for biz in businesses:
                            ...
                            if biz["categories"]:
                                accepted, _ = is_home_garden_business(biz["categories"])
                                if not accepted:
                        ...
                        for biz in candidates:
                            ...
                            if detail_html:
                                ...
                                if real_cats:
                                    ...
                                    accepted, reject_reason = is_home_garden_business(real_cats)
                                    if not accepted:
                                        # Save to shared rejected file if complete data present
                                        img = biz.get("image_url", "")
                                        if (biz.get("phone") and biz.get("website") and
                                                biz.get("address") and img and
                                                "yelp_og_image" not in img):
                                            clean_addr = re.sub(
                                                r"(Open|Closed)\s+(until|now).*?(AM|PM)\s*", "",
                                                biz["address"], flags=re.I).strip()
                                            rejected_records.append({
                                                ...
                                                "phone": biz["phone"],
                                                "website": biz["website"],
                                                "address": clean_addr or biz["address"],
                                                "rating": biz.get("rating", ""),
                                                "profile_url": biz["profile_url"],
                                                "image_url": img,
                                                "rejected_from": CATEGORY_SLUG,
                                    ...
                                    # No categories found on detail page — reject, we can't verify
                ...
                # Build directory rows + audit records
                for biz in selected:
                    ...
                    audit_records.append(make_audit_record(state, city, biz))
            ...
            if idx % 5 == 0 or idx == len(remaining):
                ...
                save_audit(audit_records)
                save_rejected(rejected_records)
                log.info(f"  [Checkpoint] {len(done)} cities done, {len(all_rows)} total rows, {len(rejected_records)} rejected saved")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    suspicious_count = sum(1 for r in audit_records if r.get("suspicious"))
    log.info(f"\n{'='*60}")
    log.info(f"DONE")
    log.info(f"  Cities scraped: {len(done)}")
    log.info(f"  Yelp rows: {len(all_rows)}")
    log.info(f"  Business pages visited: {biz_pages_visited}")
    log.info(f"  Suspicious rows to review: {suspicious_count}")
    log.info(f"  Rejected businesses saved: {len(rejected_records)}")
    log.info(f"  Audit file: {AUDIT_FILE}")
    log.info(f"  Rejected file: {REJECTED_FILE}")
    log.info(f"{'='*60}")
